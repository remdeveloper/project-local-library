...
function booksBorrowedCount(books) {
  let counter = 0 
  console.log(books)
  for (let book of books){  //changing book in to book of fixed it because books was an array for some reason   
...
let mappedKeys = sortedKeys.map(key =>{
  ...
  return {name: `${aut.name.first} ${aut.name.last}`, count: countedBorrows[key]}    
